...
export default function PostedJobs() {
  ...
  const [filterType, setFilterType] = useState("all"); // all, On-site, Remote, Hybrid
  const [filterStatus, setFilterStatus] = useState("all"); // all, active, expired
  const [sortBy, setSortBy] = useState("newest"); // newest, oldest, applications, openings
  ...
  const stats = useMemo(() => {
    ...
    const totalOpenings = postedJobs.reduce(
    ...
    <div className="min-h-screen bg-gray-50 pb-12">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
          <StatsCard label="Total Jobs" value={stats.total} icon="💼" color="blue" />
          <StatsCard
            label="Active Jobs"
            value={stats.active}
            icon="✅"
    ...
    <div className="bg-white shadow-sm rounded-xl p-6 border border-gray-200 hover:shadow-lg transition group">
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1 min-w-0">
          <h2 className="text-xl font-semibold text-gray-900 mb-1 group-hover:text-blue-600 transition truncate">
            {job.title}
          </h2>
      ...
      <div className="space-y-2 mb-4">
        <InfoRow icon="📍" text={job.jobLocation} />
        <InfoRow icon="👥" text={`${job.openings} openings`} />
        <InfoRow icon="💼" text={job.empType} />
        <InfoRow icon="🎓" text={job.experience} />
        <InfoRow icon="💰" text={job.salary} />
      </div>

      {/* Deadline */}
      {job.deadline && (
        <div
          className={`text-sm mb-4 pb-4 border-b border-gray-100 ${
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className="flex-1 bg-red-50 text-red-600 font-medium py-2.5 rounded-lg hover:bg-red-100 transition border border-red-200"
        >
          Delete
        </button>
      </div>
